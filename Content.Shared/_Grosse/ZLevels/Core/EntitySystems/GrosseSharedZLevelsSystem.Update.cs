...
public abstract partial class GrosseSharedZLevelsSystem
{
    ...
    private void ProcessZPhysics(Entity<GrosseZPhysicsComponent, PhysicsComponent> entity, float frameTime)
    {
        ...
        {
            var velocityEvent = new GrosseGetZVelocityEvent((entity, zPhysicsComponent));
        ...
        if (zPhysicsComponent is { Velocity: < 0, Fallable: true })
        {
            if (distanceToGround <= 0.05f)
            {
                ...
                {
                    var hitEv = new GrosseZLevelHitEvent(-zPhysicsComponent.Velocity);
        ...
        if (zPhysicsComponent.LocalPosition < 0)
        {
            if (TryMoveDownOrChasm(entity))
            {
                ...
                {
                    var fallEv = new GrosseZLevelFallMapEvent();
        ...
        if (float.Abs(oldVelocity - zPhysicsComponent.Velocity) > 0.001f)
            DirtyField(entity, zPhysicsComponent, nameof(GrosseZPhysicsComponent.Velocity));

        if (float.Abs(oldHeight - zPhysicsComponent.LocalPosition) > 0.001f)
            DirtyField(entity, zPhysicsComponent, nameof(GrosseZPhysicsComponent.LocalPosition));
        ...
        if (zPhysicsComponent.VelocityGravity)
        {
            ...
            if (entity.Comp2.BodyStatus != targetStatus)
            {
                ...
                var statusEv = new GrosseZBodyStatusChangedEvent(targetStatus);
    ...
    private void SleepUpdate(Entity<GrosseZPhysicsComponent> entity, float frameTime)
    {
...
/// </summary>
[ByRefEvent]
public readonly record struct GrosseZBodyStatusChangedEvent(BodyStatus NewStatus);
