...
public static class GrosseFilter
{
    ...
    public static Filter ZPvsExcept(EntityUid origin, IEntityManager? entManager = null)
    {
        ...
        var zSystem = entManager.System<GrosseSharedZLevelsSystem>();
        ...
        for (var i = 1; i <= GrosseSharedZLevelsSystem.MaxZLevelsAboveRendering; i++)
        {
            if (zSystem.TryMapOffset(currentMap, i, out var mapAbove))
                visibleMap.Add(mapAbove);
        }

        for (var i = 1; i <= GrosseSharedZLevelsSystem.MaxZLevelsBelowRendering; i++)
        {
