...
using Content.Shared._Grosse.ZLevels.Mapping.Prototypes;
...
public sealed partial class GrosseMappingZNetworkCommand : LocalizedEntityCommands
{
    ...
    public override string Description => "Load GrosseZLevelMapPrototype as ZNetwork for mapping";


    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        var options = new List<CompletionOption>();
        foreach (var map in _proto.EnumeratePrototypes<GrosseZLevelMapPrototype>())
        {
            options.Add(new CompletionOption(map.ID));
        }

        return CompletionResult.FromHintOptions(options, "GrosseZLevelMapPrototype");
    }
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        //Get Map Prototype
        if (!_proto.Resolve<GrosseZLevelMapPrototype>(args[0], out var indexedZMap))
        {
            shell.WriteError($"Unknown GrosseZLevelMapPrototype {args[0]}");
            return;
        }
