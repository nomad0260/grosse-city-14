...
using Content.Server._Grosse.ZLevels.Core.Components;
...
public sealed partial class GrosseGameMapMappingZNetworkCommand : LocalizedEntityCommands
{
    ...
    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        ...
        {
            // Check if any station in this map has GrosseStationZLevelsComponent  component with maps
            var hasZLevels = false;
            foreach (var station in map.Stations.Values)
            {
                if (station.StationComponentOverrides.TryGetComponent<GrosseStationZLevelsComponent>(_compFactory, out var zNetwork))
                {
        ...
        return CompletionResult.FromHintOptions(options, "GameMapPrototype with GrosseStationZLevelsComponent");
    }
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        // Find the first station with GrosseStationZLevelsComponent
        GrosseStationZLevelsComponent? zNetwork = null;
        foreach (var station in mapProto.Stations.Values)
        {
            if (station.StationComponentOverrides.TryGetComponent<GrosseStationZLevelsComponent>(_compFactory, out var zNet))
            {
                zNetwork = zNet;
                break;
            }
        ...
        if (zNetwork == null)
        {
            shell.WriteError($"No station with GrosseStationZLevelsComponent  found in map {mapProto.ID}");
            return;
        }
