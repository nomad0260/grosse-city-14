...
///
/// Geometry model: each network designates one <see cref="GrosseZGridNetworkComponent.AnchorGrid"/>
/// (static planet if present, otherwise the lowest <see cref="EntityUid"/> for determinism). Every
/// other grid stores its pose as <see cref="GrosseZGridComponent.NetworkOffset"/>/
/// <see cref="GrosseZGridComponent.NetworkRotation"/> relative to that anchor. The anchor is the single
...
/// </summary>
public sealed partial class GrosseZGridSyncSystem : VirtualController
{
    public override void Initialize()
    {
        base.Initialize();
        SubscribeLocalEvent<GrosseZGridComponent, GrosseGridAddedIntoZNetworkEvent>(OnGridLinked);
        SubscribeLocalEvent<GrosseZGridComponent, GrosseGridRemovedFromZNetworkEvent>(OnGridUnlinked);
        SubscribeLocalEvent<GrosseZGridComponent, MoveEvent>(OnGridMoved);
        SubscribeLocalEvent<GrosseZGridComponent, MassDataChangedEvent>(OnMassChanged);
    }
    [Dependency] private SharedTransformSystem _transform = default!;
    [Dependency] private GrosseZLevelsSystem _zlevels = default!;
    [Dependency] private GrosseZGridConnectorSystem _connectorSystem = default!;

    [Dependency] private EntityQuery<GrosseZGridComponent> _gridCompQuery = default!;
    ...
    private void OnGridLinked(Entity<GrosseZGridComponent> zGridEnt, ref GrosseGridAddedIntoZNetworkEvent ev)
    {
    ...
    private void OnGridUnlinked(Entity<GrosseZGridComponent> ent, ref GrosseGridRemovedFromZNetworkEvent ev)
    {
    ...
    private void OnGridMoved(Entity<GrosseZGridComponent> ent, ref MoveEvent ev)
    {
    ...
    private void OnMassChanged(Entity<GrosseZGridComponent> ent, ref MassDataChangedEvent args)
    {
    ...
    private void ApplyAnchorToGrid(EntityUid gridUid, GrosseZGridComponent comp, Vector2 anchorPos, Angle anchorRot)
    {
    ...
    private EntityUid ChooseAnchor(GrosseZGridNetworkComponent net)
    {
    ...
    private bool HasValidAnchor(GrosseZGridNetworkComponent net)
    {
    ...
    private void EnsureAnchor(GrosseZGridNetworkComponent net)
    {
    ...
    /// </summary>
    private void RebaseToAnchor(GrosseZGridNetworkComponent net, EntityUid newAnchor)
    {
    ...
    private void RecalculateNetworkCache(Entity<GrosseZGridNetworkComponent> network)
    {
        var totalMass = 0f;
        var hasStatic = false;
    ...
    private void SnapNewGridToAnchor(GrosseZGridNetworkComponent net, Entity<GrosseZGridComponent> grid)
    {
    ...
    private void RunRigidBodyConsensus(GrosseZGridNetworkComponent net)
    {
    ...
    public override void UpdateBeforeSolve(bool prediction, float frameTime)
    {
        ...
        var netEnum = EntityQueryEnumerator<GrosseZGridNetworkComponent>();
    ...
    private void CorrectFlyingNetwork(GrosseZGridNetworkComponent net)
    {
        RunRigidBodyConsensus(net);
