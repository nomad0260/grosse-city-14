...
public sealed partial class GrosseZLevelsSystem : GrosseSharedZLevelsSystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<GrosseZMapComponent, EntityTerminatingEvent>(OnZMapTerminating);
        SubscribeLocalEvent<GrosseStationZLevelsComponent, StationPostInitEvent>(OnStationPostInit);

        InitView();
    }
    private void OnZMapTerminating(Entity<GrosseZMapComponent> ent, ref EntityTerminatingEvent args)
    {
        ...
        if (TerminatingOrDeleted(networkUid) || !TryComp<GrosseZMapNetworkComponent>(networkUid, out var network))
            return;

        TryRemoveMapsFromNetwork((networkUid, network), new[] { ent.Owner });
    }
    private void OnStationPostInit(Entity<GrosseStationZLevelsComponent> ent, ref StationPostInitEvent args)
    {
    ...
    /// </summary>
    [PublicAPI]
    public void InitializeZNetwork(Entity<GrosseZMapNetworkComponent> network)
    {
