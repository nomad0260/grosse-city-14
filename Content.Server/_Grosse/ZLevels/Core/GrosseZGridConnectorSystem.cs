...
/// <summary>
/// Universal z-grid network recalculator driven by <see cref="GrosseZGridConnectorComponent"/>.
...
/// </summary>
public sealed partial class GrosseZGridConnectorSystem : EntitySystem
{
    public override void Initialize()
    {
        base.Initialize();
        SubscribeLocalEvent<GrosseZGridConnectorComponent, MapInitEvent>(OnConnectorMapInit);
        SubscribeLocalEvent<GrosseZGridConnectorComponent, AnchorStateChangedEvent>(OnConnectorAnchorChanged);
        SubscribeLocalEvent<GrosseZGridConnectorComponent, EntityTerminatingEvent>(OnConnectorTerminating);
        SubscribeLocalEvent<GrosseZGridComponent, EntityTerminatingEvent>(OnGridTerminating);
        SubscribeLocalEvent<GrosseZGridNetworkComponent, ComponentShutdown>(OnGridNetworkShutdown);
    }
    ...
    private void OnConnectorMapInit(Entity<GrosseZGridConnectorComponent> ent, ref MapInitEvent args)
    {
        _dirty = true;
    }
    private void OnConnectorAnchorChanged(Entity<GrosseZGridConnectorComponent> ent, ref AnchorStateChangedEvent args)
    {
        _dirty = true;
    }
    private void OnConnectorTerminating(Entity<GrosseZGridConnectorComponent> ent, ref EntityTerminatingEvent args)
    {
        _dirty = true;
    }
    private void OnGridTerminating(Entity<GrosseZGridComponent> ent, ref EntityTerminatingEvent args)
    {
        _dirty = true;
    }
    ...
    private void OnGridNetworkShutdown(Entity<GrosseZGridNetworkComponent> ent, ref ComponentShutdown args)
    {
    ...
    /// minimal delta: survivors keep their pose, only joined grids fire <see cref="GrosseGridAddedIntoZNetworkEvent"/>
    /// and only departed grids fire <see cref="GrosseGridRemovedFromZNetworkEvent"/>. Networks are reused (never
    ...
    private void RecalculateGridNetworks()
    {
        ...
        _removeBuffer.Clear();
        var nq = EntityQueryEnumerator<GrosseZGridNetworkComponent>();
    ...
    private void ComputeDesiredComponents()
    {
        ...
        var query = EntityQueryEnumerator<GrosseZGridConnectorComponent, TransformComponent>();
