...
public sealed partial class GrosseZLevelsLaddersCacheSystem : EntitySystem
{
    public override void Initialize()
    {
        base.Initialize();
        SubscribeLocalEvent<GrosseZLevelHighGroundComponent, MapInitEvent>(OnLadderInit);
        SubscribeLocalEvent<GrosseZLevelHighGroundComponent, ComponentShutdown>(OnLadderShutdown);
        SubscribeLocalEvent<GrosseZLevelHighGroundComponent, AnchorStateChangedEvent>(OnLadderAnchorChanged);
    }
    ...
    [Dependency] private EntityQuery<GrosseZLevelsLaddersCacheComponent> _cacheQuery = default!;
    private void OnLadderInit(Entity<GrosseZLevelHighGroundComponent> ent, ref MapInitEvent args)
    {
        TryCacheLadder(ent);
    }
    private void OnLadderShutdown(Entity<GrosseZLevelHighGroundComponent> ent, ref ComponentShutdown args)
    {
        TryRemoveCache(ent);
    }
    private void OnLadderAnchorChanged(Entity<GrosseZLevelHighGroundComponent> ent, ref AnchorStateChangedEvent args)
    {
        if (args.Anchored)
            TryCacheLadder(ent);
        else
    ...
    /// </summary>
    private void TryCacheLadder(Entity<GrosseZLevelHighGroundComponent> ent)
    {
        ...
        var cache = EnsureComp<GrosseZLevelsLaddersCacheComponent>(gridUid.Value);
        ...
        cache.Slopes[tilePos] = new GrosseCachedSlope
        {
            Entity = ent,
            Direction = dir,
        };
    ...
    /// </summary>
    private void TryRemoveCache(Entity<GrosseZLevelHighGroundComponent> ent)
    {
        if (!_xformQuery.TryGetComponent(ent, out var xform))
            return;
