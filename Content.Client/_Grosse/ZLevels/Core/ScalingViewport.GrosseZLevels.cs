...
public sealed partial class ScalingViewport
{
    ...
    private GrosseClientZLevelsSystem? _zLevels;
    ...
    private void RenderZLevels(IClydeViewport viewport)
    {
        ...
        _zLevels ??= _entityManager.System<GrosseClientZLevelsSystem>();
        _mapSystem ??= _entityManager.System<SharedMapSystem>();

        if (_player.LocalEntity is null)
            return;

        if (!_entityManager.TryGetComponent<GrosseZLevelViewerComponent>(_player.LocalEntity.Value, out var zLevelViewer))
            return;
        ...
        var lowestDepth = 0;
        for (var i = 0; i >= -GrosseSharedZLevelsSystem.MaxZLevelsBelowRendering; i--)
        {
        ...
        for (var depth = lowestDepth; depth <= lookUp; depth++)
        {
            ...
            else
            {
                ...
                var offset = rotation.ToWorldVec() * GrosseClientZLevelsSystem.ZLevelOffset * depth;
