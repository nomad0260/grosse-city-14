...
namespace Content.Client._Grosse.ZLevels.Core.Overlays;

public sealed partial class GrosseZLevelDebugOverlay : Overlay
{
    ...
    public GrosseZLevelDebugOverlay()
    {
        IoCManager.InjectDependencies(this);

        _zLevels = _entityManager.System<GrosseSharedZLevelsSystem>();
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        {
            if (!_entityManager.TryGetComponent<GrosseZPhysicsComponent>(uid, out var zPhys) ||
        ...
        var gridQuery = _entityManager.EntityQueryEnumerator<GrosseZGridComponent, TransformComponent>();
    ...
    private void DrawEntityDebug(in OverlayDrawArgs args, EntityUid uid, GrosseZPhysicsComponent component)
    {
...
public sealed partial class GrosseShowZLevelDebugCommand : LocalizedCommands
{
    ...
    {
        if (_overlayManager.HasOverlay<GrosseZLevelDebugOverlay>())
        {
            _overlayManager.RemoveOverlay<GrosseZLevelDebugOverlay>();
            return;
        }

        _overlayManager.AddOverlay(new GrosseZLevelDebugOverlay());
    }
}
