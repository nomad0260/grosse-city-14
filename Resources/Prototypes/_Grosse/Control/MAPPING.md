...
`classes` наследуется с дописыванием (`AlwaysPushInheritance`). Другой набор целиком — без `parent`.

Новая фракция: свой `controlTeam` и ссылка в `StationControlConfig`. Не копируй геймрул.
